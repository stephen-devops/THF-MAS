...
class MapRelationshipsSchema(BaseModel):
    ...
    source_type: EntityType = Field(description="Type of source entity (the subject performing the action). PROCESS: 'what did process X spawn/create/inject?' USER: 'what did user X launch/access?' HOST: 'who logged into host X?', 'what runs on X?' FILE: 'what created file X?'")
    ...
    target_type: Optional[EntityType] = Field(default=None, description="Type of target entity (the object receiving the action). Examples: process->process (spawn/inject), process->file (create/modify), user->process (launch), user->host (access), host->user (logins). Omit for all relationship types.")
